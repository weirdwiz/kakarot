...
function CollapsibleSection({ title, children, defaultOpen = true, icon, badge }: CollapsibleSectionProps) {
  ...
  return (
    <div className="border-b border-slate-200 dark:border-edge last:border-b-0">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center gap-2 py-3 px-1 text-left hover:bg-input dark:hover:bg-edge/50 transition-colors rounded-lg group"
      >
        ...
        {badge !== undefined && badge > 0 && (
          <span className="text-xs bg-edge dark:bg-edge text-slate-600 dark:text-slate-400 px-2 py-0.5 rounded-full">
            {badge}
          </span>
        )}
      </button>
    ...
    <div className="relative group">
      ...
      >
        {hasSubBullets ? (
          ...
          <span className="text-slate-500 dark:text-slate-500 mt-1.5 flex-shrink-0 w-1.5 h-1.5 rounded-full bg-slate-400 dark:bg-edge" />
        )}
        ...
        {text.length > 15 && (
          <button
            ...
            className={`flex-shrink-0 p-1 rounded-full transition-all duration-200 ${
              isHovered || isOpen
                ? 'opacity-100 bg-accent/20 dark:bg-accent/20 hover:bg-accent/30 dark:hover:bg-accent/30'
                : 'opacity-0 pointer-events-none'
            }`}
            title="Deep Dive"
          >
            <Search className={`w-3.5 h-3.5 text-accent dark:text-accent ${isLoading ? 'animate-pulse' : ''}`} />
          </button>
        )}
      </div>
      ...
      {isOpen && (
        <div
          ref={popoverRef}
          className="fixed z-[9999] w-[480px] max-h-[450px] rounded-xl border border-edge bg-edge shadow-2xl flex flex-col"
          style={{
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {result.notableQuotes.map((quote, idx) => (
                        <div
                          key={idx}
                          className="bg-input border border-edge rounded-lg p-3"
                        >
      ...
      {hasSubBullets && isExpanded && (
        <div className="ml-4 border-l border-slate-200 dark:border-edge pl-3">
          {subBullets.map((subBullet, idx) => (
            <div key={idx} className="flex items-start gap-2 py-1">
              <span className="text-slate-400 dark:text-slate-600 mt-1.5 flex-shrink-0 w-1 h-1 rounded-full bg-slate-300 dark:bg-accent" />
            ...
            {notes.decisions.map((decision, idx) => (
              <div key={idx} className="space-y-1">
                ...
                {decision.rationale && decision.rationale.length > 0 && (
                  <div className="ml-5 pl-3 border-l border-slate-200 dark:border-edge">
                    ...
                    {decision.rationale.map((reason, reasonIdx) => (
                      <div key={reasonIdx} className="flex items-start gap-2 py-0.5">
                        <span className="text-slate-400 dark:text-slate-600 mt-1.5 w-1 h-1 rounded-full bg-slate-300 dark:bg-accent flex-shrink-0" />
        ...
        >
          <div className="space-y-2">
            {notes.actionItems.map((item, idx) => (
              <div
                key={idx}
                className="flex items-start gap-3 py-2 px-3 bg-input dark:bg-input border border-slate-200 dark:border-edge rounded-lg"
              >
