...
function CollapsibleSection({ title, children, defaultOpen = true, icon, badge }: CollapsibleSectionProps) {
  ...
  return (
    <div className="border-b border-slate-200 dark:border-[#1A1A1A] last:border-b-0">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center gap-2 py-3 px-1 text-left hover:bg-slate-100 dark:hover:bg-[#1A1A1A]/50 transition-colors rounded-lg group"
      >
        ...
        {badge !== undefined && badge > 0 && (
          <span className="text-xs bg-slate-200 dark:bg-[#2A2A2A] text-slate-600 dark:text-slate-400 px-2 py-0.5 rounded-full">
            {badge}
          </span>
        )}
      </button>
    ...
    <div className="relative group">
      ...
      >
        {hasSubBullets ? (
          ...
          <span className="text-slate-500 dark:text-slate-500 mt-1.5 flex-shrink-0 w-1.5 h-1.5 rounded-full bg-slate-400 dark:bg-slate-500" />
        )}
        ...
        {text.length > 15 && (
          <button
            ...
            className={`flex-shrink-0 p-1 rounded-full transition-all duration-200 ${
              isHovered || isOpen
                ? 'opacity-100 bg-purple-100 dark:bg-purple-500/20 hover:bg-purple-200 dark:hover:bg-purple-500/30'
                : 'opacity-0 pointer-events-none'
            }`}
            title="Deep Dive"
          >
            <Search className={`w-3.5 h-3.5 text-purple-600 dark:text-purple-400 ${isLoading ? 'animate-pulse' : ''}`} />
          </button>
        )}
      </div>
        ...
        <div className="ml-4 border-l border-slate-200 dark:border-[#2A2A2A] pl-3">
          {subBullets.map((subBullet, idx) => (
            <div key={idx} className="flex items-start gap-2 py-1">
              <span className="text-slate-400 dark:text-slate-600 mt-1.5 flex-shrink-0 w-1 h-1 rounded-full bg-slate-300 dark:bg-slate-600" />
                  ...
                  <div className="ml-5 pl-3 border-l border-slate-200 dark:border-[#2A2A2A]">
                    ...
                    {decision.rationale.map((reason, reasonIdx) => (
                      <div key={reasonIdx} className="flex items-start gap-2 py-0.5">
                        <span className="text-slate-400 dark:text-slate-600 mt-1.5 w-1 h-1 rounded-full bg-slate-300 dark:bg-slate-600 flex-shrink-0" />
        ...
        >
          <div className="space-y-2">
            {notes.actionItems.map((item, idx) => (
              <div
                key={idx}
                className="flex items-start gap-3 py-2 px-3 bg-slate-100 dark:bg-[#0F0F10] border border-slate-200 dark:border-[#1A1A1A] rounded-lg"
              >
